...
class Order {
    ...
    async getFilteredOrder(server, payload, bearerToken) {
        let dbClient;
        ...
        `;
            ...
            const purchaseOrder = result.rows.map(row => ({
                ...row,
                c_order_id: parseInt(row.c_order_id, 10)
            }));
            ...
            // --- FILTERING NODES (JS) ---
            const reqIds = purchaseOrder.map(r => r.c_order_id);
